...
var originalFlavors2 = ["Banana Nut Fudge",
    "Black Walnut",
    "Burgundy Cherry",
   "Butterscotch Ribbon",
    "Cherry Macaron",
...
function is31Flavors(flavors){
  var arrayLength = (flavors.length)
  if (arrayLength === 31) {
    return "TRUE"
  }
  else {
...
var x = is31Flavors(originalFlavors)
var y = is31Flavors(originalFlavors2)
console.log(x)
console.log(y)
...
function copy(arr) {
    let trueOriginalFlavors = [...arr];
    return trueOriginalFlavors;
}
...
function filterByWord(arr, flavorX) {
    let filteredArray = [];
    for (let i = 0; i < arr.length; i++) {
        if (arr[i].includes(flavorX)) {
